...
file_path = "/media/actimetre/Project05/Actim0065-1A_2026-03-26_154719.csv"
# file_path = "/media/actimetre/Project03/Actim0015-1A_2026-03-25_155313.csv"
num_lines = 150  # Nombre de lignes à récupérer à chaque fois

# Configuration graphique
interval_seconds = 0.100  # en secondes
fenetre_temps = 1  # Fenêtre de temps affichée (en secondes)
max_points = 1000    # Nombre max de points stockés
t_start = 0.0
...
buffer_y3 = deque(maxlen=max_points)

# Intervalle de temps entre chaque récupération (en secondes)
interval_seconds = 0.1


def get_last_lines(ssh_client, file_path, num_lines):
    """Récupère les dernières lignes d'un fichier via SSH."""
...
def update(frame):
    ...
    if len(times)>0:
        ...
        y3_show = np.array(buffer_y3)
        line1.set_data(x_show, y1_show)
        line2.set_data(x_show, y2_show)
        line3.set_data(x_show, y3_show)
        # On ne garde que les points dans la fenêtre
        # ax.set_xlim(x_show[-1]-fenetre_temps, x_show[-1])

        # Attente avant la prochaine récupération
...
ax.set_title(f"Oscilloscope en temps réel (fenêtre glissante de {fenetre_temps}s)")
ax.grid(True)
    
ax2 = ax.twinx()
line3, = ax2.plot([], [], 'g-', lw=1)
